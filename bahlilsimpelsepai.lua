...
-- ============================================================
-- WIND UI THEME - Color Palette
-- ============================================================
-- Primary Background:   RGB(15, 15, 20)    - Deep dark base
-- Secondary Background: RGB(22, 22, 30)    - Elevated surface
-- Panel Background:     RGB(28, 28, 38)    - Side panels
-- Accent Color:         RGB(99, 102, 241)  - Indigo/violet accent
-- Accent Hover:         RGB(129, 140, 248) - Lighter accent
-- Border Color:         RGB(45, 45, 60)    - Subtle borders
-- Text Primary:         RGB(245, 245, 255) - Main text
-- Text Secondary:       RGB(160, 163, 189) - Muted text
-- Success:              RGB(52, 211, 153)  - Green
-- Warning:              RGB(251, 191, 36)  - Amber
-- ============================================================

local WindAccent      = Color3.fromRGB(99, 102, 241)
local WindAccentLight = Color3.fromRGB(129, 140, 248)
local WindBg          = Color3.fromRGB(15, 15, 20)
local WindSurface     = Color3.fromRGB(22, 22, 30)
local WindPanel       = Color3.fromRGB(28, 28, 38)
local WindBorder      = Color3.fromRGB(45, 45, 60)
local WindTextPrimary = Color3.fromRGB(245, 245, 255)
local WindTextMuted   = Color3.fromRGB(160, 163, 189)
local WindSuccess     = Color3.fromRGB(52, 211, 153)
local WindDanger      = Color3.fromRGB(248, 113, 113)
local WindButton      = Color3.fromRGB(35, 35, 50)
local WindButtonHover = Color3.fromRGB(50, 50, 70)
local WindTopBar      = Color3.fromRGB(18, 18, 25)
local WindCodeBg      = Color3.fromRGB(12, 12, 18)

-- ============================================================
-- GUI CONSTRUCTION - Wind UI Theme
-- ============================================================
...
local BgCorner = Create("UICorner",{Parent = Background, CornerRadius = UDim.new(0, 10)})
-- Thin border stroke
local BgStroke = Create("UIStroke",{
    Parent = Background,
    Color = WindBorder,
    Thickness = 1,
    Transparency = 0.4
...
local LeftPanel = Create("Frame",{
    Parent = Background,
    BackgroundColor3 = WindPanel,
    BorderSizePixel = 0,
    Position = UDim2.new(0, 0, 0, 32),
    Size = UDim2.new(0, 131, 0, 236)
})
local LeftPanelCorner = Create("UICorner",{Parent = LeftPanel, CornerRadius = UDim.new(0, 6)})

-- Separator line between left and right panel
local PanelDivider = Create("Frame",{
    Parent = Background,
    BackgroundColor3 = WindBorder,
    BorderSizePixel = 0,
    Position = UDim2.new(0, 130, 0, 32),
    Size = UDim2.new(0, 1, 0, 236)
})

-- Remote log list (scrolling)
local LogList = Create("ScrollingFrame",{
    ...
    Size = UDim2.new(0, 131, 0, 220),
    CanvasSize = UDim2.new(0, 0, 0, 0),
    ScrollBarThickness = 3,
    ScrollBarImageColor3 = WindAccent
})
...
local RightPanel = Create("Frame",{
    Parent = Background,
    BackgroundColor3 = WindSurface,
    BorderSizePixel = 0,
    Position = UDim2.new(0, 131, 0, 32),
    Size = UDim2.new(0, 319, 0, 236)
})
local RightPanelCorner = Create("UICorner",{Parent = RightPanel, CornerRadius = UDim.new(0, 6)})

-- Code box
local CodeBox = Create("Frame",{
    Parent = RightPanel,
    BackgroundColor3 = WindCodeBg,
    BorderSizePixel = 0,
    Size = UDim2.new(0, 319, 0, 119)
})
local CodeBoxCorner = Create("UICorner",{Parent = CodeBox, CornerRadius = UDim.new(0, 4)})

-- Code box top accent bar
local CodeBoxAccent = Create("Frame",{
    Parent = CodeBox,
    BackgroundColor3 = WindAccent,
    BorderSizePixel = 0,
    Position = UDim2.new(0, 0, 0, 0),
    Size = UDim2.new(1, 0, 0, 2)
})

-- Function buttons scrolling frame
...
local UIGridLayout = Create("UIGridLayout",{
    ...
    CellSize = UDim2.new(0, 92, 0, 25)
})

-- Top Bar
local TopBar = Create("Frame",{
    Parent = Background,
    BackgroundColor3 = WindTopBar,
    BorderSizePixel = 0,
    Size = UDim2.new(0, 450, 0, 32)
})
local TopBarCorner = Create("UICorner",{Parent = TopBar, CornerRadius = UDim.new(0, 10)})

-- Bottom clipping fix for TopBar corners (makes bottom of topbar flat)
...
-- Colored accent dot on the left of topbar
...
local AccentDotCorner = Create("UICorner",{Parent = AccentDot, CornerRadius = UDim.new(1, 0)})

-- SimpleSpy title button (toggle)
...
local CloseCorner = Create("UICorner",{Parent = CloseButton, CornerRadius = UDim.new(1,0)})
...
-- Maximize/side toggle button
local MaximizeButton = Create("TextButton",{
    Parent = TopBar,
    BackgroundColor3 = WindSuccess,
    BackgroundTransparency = 0.3,
    BorderSizePixel = 0,
...
local MaximizeCorner = Create("UICorner",{Parent = MaximizeButton, CornerRadius = UDim.new(1,0)})

-- Maximize icon
local MaximizeIcon = Create("TextLabel",{
...
local MinimizeCorner = Create("UICorner",{Parent = MinimizeButton, CornerRadius = UDim.new(1,0)})
...
local ToolTipCorner = Create("UICorner",{Parent = ToolTip, CornerRadius = UDim.new(0, 6)})
local ToolTipStroke = Create("UIStroke",{Parent = ToolTip, Color = WindBorder, Thickness = 1, Transparency = 0.3})
-- Accent left bar on tooltip
local ToolTipBar = Create("Frame",{
    Parent = ToolTip,
    BackgroundColor3 = WindAccent,
    BorderSizePixel = 0,
...
local ToolTipBarCorner = Create("UICorner",{Parent = ToolTipBar, CornerRadius = UDim.new(1,0)})
...
local selectedColor = WindAccent
local deselectedColor = WindSurface
...
function onToggleButtonHover()
    if not toggle then
        TweenService:Create(Simple, TweenInfo.new(0.3), {TextColor3 = WindDanger}):Play()
        TweenService:Create(AccentDot, TweenInfo.new(0.3), {BackgroundColor3 = WindDanger}):Play()
    else
        TweenService:Create(Simple, TweenInfo.new(0.3), {TextColor3 = WindSuccess}):Play()
        TweenService:Create(AccentDot, TweenInfo.new(0.3), {BackgroundColor3 = WindSuccess}):Play()
    end
end

function onToggleButtonUnhover()
    TweenService:Create(Simple, TweenInfo.new(0.3), {TextColor3 = WindTextPrimary}):Play()
    TweenService:Create(AccentDot, TweenInfo.new(0.3), {BackgroundColor3 = WindAccent}):Play()
end
...
function bringBackOnResize()
    ...
    if (currentY < 0) or (currentY > (viewportSize.Y - (closed and 32 or Background.AbsoluteSize.Y) - GuiInset.Y)) then
        if currentY < 0 then
            currentY = 0
        else
            currentY = viewportSize.Y - (closed and 32 or Background.AbsoluteSize.Y) - GuiInset.Y
        end
    end
        ...
        if not connections["drag"] then
            connections["drag"] = RunService.RenderStepped:Connect(function()
                ...
                if newPos ~= lastPos then
                    ...
                    if (currentY < 0 and currentY < currentPos.Y) or (currentY > (viewportSize.Y - (closed and 32 or Background.AbsoluteSize.Y) - GuiInset.Y) and currentY > currentPos.Y) then
                        if currentY < 0 then
                            currentY = 0
                        else
                            currentY = viewportSize.Y - (closed and 32 or Background.AbsoluteSize.Y) - GuiInset.Y
    ...
    return function()
        for i, _ in next, data do
            spawn(function()
                ...
                TweenService:Create(i, TweenInfo.new(0.4), {BackgroundTransparency = properties.BackgroundTransparency}):Play()
                if i:IsA("TextBox") or i:IsA("TextButton") or i:IsA("TextLabel") then
                    properties.TextTransparency = i.TextTransparency
                    i.TextTransparency = 1
                    TweenService:Create(i, TweenInfo.new(0.4), {TextTransparency = properties.TextTransparency}):Play()
                elseif i:IsA("ImageButton") or i:IsA("ImageLabel") then
                    properties.ImageTransparency = i.ImageTransparency
                    i.ImageTransparency = 1
                    TweenService:Create(i, TweenInfo.new(0.4), {ImageTransparency = properties.ImageTransparency}):Play()
                end
                i.Visible = true
            end)
        end
...
function toggleMinimize(override)
    ...
    if closed then
        ...
        TweenService:Create(LeftPanel, TweenInfo.new(0.4), {Size = UDim2.new(0, 131, 0, 0)}):Play()
        wait(0.4)
    else
        TweenService:Create(LeftPanel, TweenInfo.new(0.4), {Size = UDim2.new(0, 131, 0, 236)}):Play()
        wait(0.4)
...
function toggleMaximize()
    if not sideClosed and not maximized then
        ...
        disable.BackgroundColor3 = WindBg
        ...
        TweenService:Create(CodeBox, TweenInfo.new(0.4), {Size = UDim2.new(0.5, 0, 0.5, 0), Position = UDim2.new(0.25, 0, 0.25, 0)}):Play()
        TweenService:Create(disable, TweenInfo.new(0.4), {BackgroundTransparency = 0.5}):Play()
        disable.MouseButton1Click:Connect(function()
            ...
            TweenService:Create(CodeBox, TweenInfo.new(0.4), {Size = prevSize, Position = prevPos}):Play()
            TweenService:Create(disable, TweenInfo.new(0.4), {BackgroundTransparency = 1}):Play()
            wait(0.4)
            disable:Destroy()
...
function eventSelect(frame)
    if selected and selected.Log then
        if selected.Button then
            spawn(function()
                TweenService:Create(selected.Button, TweenInfo.new(0.3), {BackgroundColor3 = WindButton, BackgroundTransparency = 0}):Play()
            end)
        end
        selected = nil
    end
    ...
    if selected and selected.Log then
        spawn(function()
            TweenService:Create(frame.Button, TweenInfo.new(0.3), {BackgroundColor3 = WindAccent, BackgroundTransparency = 0.1}):Play()
        end)
...
-- ============================================================
-- Wind UI styled function button
-- ============================================================
function newButton(name, description, onClick)
    ...
    local ColorBarCorner = Create("UICorner",{Parent = ColorBar, CornerRadius = UDim.new(1,0)})
    -- Button background
    local Button = Create("TextButton",{
        Name = "Button",
        Parent = FunctionTemplate,
        BackgroundColor3 = WindButton,
        BackgroundTransparency = 0,
        BorderSizePixel = 0,
        Position = UDim2.new(0, 5, 0, 2),
        Size = UDim2.new(1, -5, 1, -4),
        AutoButtonColor = false,
        Font = Enum.Font.Gotham,
        Text = "",
        TextColor3 = WindTextPrimary,
    ...
    local ButtonCorner = Create("UICorner",{Parent = Button, CornerRadius = UDim.new(0, 4)})
    -- Button text
    ...
    Button.MouseEnter:Connect(function()
        TweenService:Create(Button, TweenInfo.new(0.15), {BackgroundColor3 = WindButtonHover}):Play()
        TweenService:Create(ColorBar, TweenInfo.new(0.15), {BackgroundColor3 = WindAccentLight}):Play()
        makeToolTip(true, description())
    end)
    Button.MouseLeave:Connect(function()
        TweenService:Create(Button, TweenInfo.new(0.15), {BackgroundColor3 = WindButton}):Play()
        TweenService:Create(ColorBar, TweenInfo.new(0.15), {BackgroundColor3 = WindAccent}):Play()
        makeToolTip(false)
    end)
    ...
    Button.MouseButton1Click:Connect(function(...)
        -- Click flash effect
        TweenService:Create(Button, TweenInfo.new(0.08), {BackgroundColor3 = WindAccent}):Play()
...
-- ============================================================
-- Wind UI styled remote log entry
-- ============================================================
function newRemote(type, data)
    ...
    local RemoteTemplate = Create("Frame",{
        ...
        BackgroundColor3 = Color3.new(1,1,1),
        BackgroundTransparency = 1,
        Size = UDim2.new(0, 117, 0, 28)
    })
    -- Color indicator (event = amber, function = indigo)
    local ColorBar = Create("Frame",{
        Name = "ColorBar",
        Parent = RemoteTemplate,
        BackgroundColor3 = (type == "event" and Color3.fromRGB(251, 191, 36)) or WindAccent,
        BorderSizePixel = 0,
    ...
    local TypeBadge = Create("TextLabel",{
        ...
        TextColor3 = (type == "event") and Color3.fromRGB(251,191,36) or WindAccentLight,
        TextSize = 7,
        TextTransparency = 0
    })
    local TypeBadgeCorner = Create("UICorner",{Parent = TypeBadge, CornerRadius = UDim.new(1,0)})
    ...
    Button.MouseEnter:Connect(function()
        ...
        TweenService:Create(Text, TweenInfo.new(0.15), {TextColor3 = WindAccentLight}):Play()
    end)
    Button.MouseLeave:Connect(function()
        TweenService:Create(Button, TweenInfo.new(0.15), {BackgroundTransparency = 1}):Play()
        TweenService:Create(Text, TweenInfo.new(0.15), {TextColor3 = WindTextPrimary}):Play()
    end)
...
if configs.supersecretdevtoggle then
    ...
    newButton("SUPER SECRET",function() return "You dont need a discription you already know what it does" end,
    function()
